...
#### Import and tidy data,dividing the discharge by 1000 to improve interpretation of the y-axis.Group by year and month, and summmarize mean discharge. 

```{r}
thebes_data <- read_csv(here("data", "thebes_discharge.csv"))
thebes_data_tidy <- pivot_longer(thebes_data,"1988":"2018",names_to="year", values_to= "Discharge") %>%
mutate(Discharge=Discharge/1000)
thebes_data_tidy
thebes_grouped_data <- thebes_data_tidy %>%
  group_by(year,Month)
thebes_grouped_data 
...
#### Add an Event column using mutate and case_when.
```{r}
thebes_events <- thebes_summarized_data %>%
  mutate(
    event = case_when(year==1993 | year == 2011 ~ 'Flood',
...
thebes_events
```
This has arranged the year and months, stating which event occured during that time. 
